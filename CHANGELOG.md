...
**Upgrading:** if you call `runGates` directly, read `result.verdict` / `result.blockedBy` instead of comparing to a string. If you call `createCartridgeApp` directly, it is now synchronous, returns `App`, and runs no gates — call `runGatePhase` first if you relied on them. If you read `renderAppWithPublication`'s `skipped` by presence, branch on `skipped.reason` — `'private'` is new and is a 401, not a csr-only partial-win — and read `fellBack.reason === 'unknown-page'` before it (the two can co-occur; an unknown url is a 404 whatever the fallback page's privacy). Three type-level changes can stop a compile: `ViewDefinition.factory` is optional (guard before calling it); `Gate.persist` may be an array (readers normalise with `Array.isArray`); `RenderWithPublicationResult.gates` is a required field (anything that constructs a result by hand adds it). Hosts that persist a page graph and replace the template's pages wholesale must carry `private` through, or the page silently becomes public. Everything else is additive.
...
### Changed
- `findEntryPage`, `resolveEntryPage`, `describeEntryResolution` and their types moved to `entry-resolution.ts`; the package barrel exports them as before, so nothing importable changed. `PageManager`'s constructor now seeds `navState` via `resolveMountEntry` and `initRouter` builds its router via `createRouter`; behaviour is byte-identical (the existing suite is unchanged) and the URL is now decoded by the same code on both sides of `createApp`.
...
- **`runGates` fails closed on a verdict it does not know.** A `mount()` that resolved anything but `'allow'` — `undefined` from a missing `return`, `void` from a redirect helper, a typo — was treated as `'allow'` and everything painted. It is now treated as `'block'`: the gate's paint stays and nothing else renders. `precheck` already behaved this way.
- `runGatePhase` narrates every server-satisfied gate up front, before the remaining gates run — a satisfied gate declared after a blocker is narrated before the block. Eager on purpose (the skip is a fact about the mount, not an event in the sequence); the docblock says so and a test pins it. `RunGatePhaseResult` is generic over `TConfig`; on a block the runtime finds the blocking gate in `applied` for teardown.
...
- **BREAKING (type-level): `ViewDefinition.factory` is optional.** Code that reads `views[].factory` and calls it stops compiling until it guards `undefined` — resolve through `getDefaultRenderResolver` / `registry.resolverFor` instead. An entry without a factory is a capability-only declaration for a page type whose factory arrives through the chunk mailbox; the resolver (`createCartridgeRegistry`, `getDefaultRenderResolver`) falls through to the mailbox for it instead of letting it shadow the chunk. From the second consumer's ask: a mailbox-only page type can now declare `csr-only` without a second cartridge.
...
- **`GateContext.navState?`** — the navigation state the mount starts on (URL > `initialNavState` > default), handed to `precheck` and `mount` by the runtime. Gates run before any App exists, so this is the only framework-supplied way for a gate on a host with no URL router to build a return URL; the redirect-round-trip docs now say so instead of pointing at an `app.getNavigationState()` that does not exist yet.
- **`Gate.persist` accepts an array of `PersistHint`** with `outcome?: 'pass' | 'fail'` — a real age gate remembers a pass across sessions and a fail for this tab. The single-hint shape is unchanged for declarations; a host that *reads* the field normalises with `Array.isArray`. Its docblock now opens with the fact the studio consumer asked us to state plainly: the framework never reads this field; it is documentation with a type. Whether it survives 1.0 as a field is on the freeze review list — this line has produced three declared-and-unread primitives (`postProcessors` until 0.9.0, `requires` until 0.10.0, and this), and the freeze should not add a fourth.

## `@airo-js/ssr` 0.11.0 — 2026-09-10

### Added
- **Private pages are refused, not published.** `renderAppWithPublication` resolves the entry page BEFORE running adapters. A `private: true` entry runs no adapter and inlines nothing; without `renderPrivate: true` it is refused with `skipped: { reason: 'private' }` (a 401 for a root-mounted host, the way `fellBack.reason === 'unknown-page'` is a 404 — nothing is substituted, so it is never a fallback); with it the page renders as HTML only. Page-private is checked before the view's `csr-only` capability and refusal wins, so a private page can never take the JSON-LD partial-win branch anonymously. `renderPrivate` is the one explicit unlock: the framework verifies nothing and reads no other input to decide, so a country- or locale-scoped host cannot open private pages by accident. **`renderAppToHTML` keeps the same promise**: it takes `renderPrivate` on its deps and refuses a private entry with `skipped: { reason: 'private' }` otherwise — the flag lives on the page graph, so both SSR entry points honour it. **`renderPrivate` also takes `{ satisfiedGates: [...] }`**: `true` is shorthand for "this render satisfies every `appliesTo: 'private'` gate" (right when the sign-in gate is the only one); a host with a second private-scoped gate names exactly the ones its render met, and the runner echoes them in `gates.satisfied`, restricted to gates that apply to the entry. The object form unlocks only when it names an enabled, applicable, `'private'`-scoped gate; a list that is empty, names only public-scoped gates (`['age']`), or names unknown ids refuses like `false` — so a list computed per request cannot open a private page on a branch where no session was verified.
- **`gates: { pending, satisfied }` on the result** (a required field; both arrays possibly empty) — what the client mount will do about gates for this entry page, via the same `selectGates` the runtime uses. `pending` for hosts that ship `data-airo-gate="pending"` and hide under it (the no-paint case); `satisfied` = the `appliesTo: 'private'` gates a `renderPrivate` render already met, for `mountCartridge({ satisfiedGates })`. When no entry page resolves at all, `pending` lists every enabled gate — the client fails closed and runs them all, and the host is told so rather than told nothing.

### Changed
- **`fellBack` can accompany a private skip.** When the requested id was rejected and the default entry it fell back to is private, the result carries both `fellBack.reason === 'unknown-page'` and `skipped.reason === 'private'`. Read `fellBack` first: an unknown url is a 404 whatever the fallback page's privacy; reading `skipped` first answers 401 and tells a crawler the url exists. The docblock, best-practices §4.9 and the example's server now say and do this. The private refusal logs under `phase: 'private-page'`, distinct from the view's `'capability-gate'`.
...
- Remounts through `update()` / `updatePages()` re-run every gate, as before, and a blocked remount throws the documented error — now pinned by a regression test the suite had explicitly deferred. **After a gate-blocked remount the next `update()` / `updatePages()` remounts**, re-running the gates, instead of hot-swapping into the destroyed App and reporting `{ mode: 'hot-swap' }` with the gate's panel still on screen.
- **A blocking gate's `destroy()` now runs** when the host calls the mount result's `destroy()` and before a remount re-runs the gate phase. `runGates` deliberately leaves a blocking gate's paint and listeners in place; until now nothing ever ended that lifetime, and a gate with a `document`-level listener leaked it past teardown.
- **The hydrate snapshot is taken only when a gate could paint** (enabled, applicable to the entry, not server-satisfied), and only on the initial mount — a mount with nothing to restore no longer serialises the render root twice. Attach host observers to the render root, not to nodes inside it, when a painting gate is in play: the restore re-parses the markup (documented in `gate.ts`).
- **Transformers see the resolved entry page.** `TransformerContext.navState.page` is the page this mount starts on — the same page the gates were scoped against — not the template's first page. A transformer that shapes the snapshot per page saw the wrong page on every deep-linked mount.
- **`private` is a structural page key.** Flipping it through `updatePages()` remounts (and re-selects gates) like `enabled` or `parent`, even when a host lists it in `pageHotSwapKeys`.
- A graph with no enabled entry page resolves `data-airo-gate="pending"` to `"error"` before reporting `onError('mount')`, so a host that hid under it is not hidden forever on a broken graph.
- The gate phase is async and the URL can change under it; if the page the App mounts differs from the page the gates were scoped against, the runtime narrates it (`log.warn`, phase `'gate'`) rather than claim the two agree by construction. The stronger fix — making the runtime's resolution authoritative for `PageManager` — is on the 1.0 list.
...
### Cost
- **The closure reachable from `mountCartridge` grows by ~1.8 KB minified / ~0.57 KB gzip** (26,531 → 28,332 B / 8,679 → 9,252 B; esbuild, ESM, es2022). Two parts. The reshape itself is ~1.0 KB / ~0.27 KB: `mountCartridge` +454 B (gate phase, hydrate stash/restore, the attribute, `satisfiedGates`); the shared entry ladder +399 B net (`mount-entry` +744 B and `entry-resolution` +331 B, `PageManager` −676 B); the gate machinery +137 B net (`gate-phase` +546 B, `run-gates` +302 B for the result object and events, `createCartridgeApp` −711 B); everything else +72 B — the figure both consumers measured against the yalc build. The pre-landing review's fixes add the other ~0.8 KB / ~0.3 KB: the blocking gate's teardown, the blocked-remount handling, the entry re-resolution after the gate phase, the conditional hydrate snapshot, `GateContext.navState`. It is code that now runs on every mount — the gate phase and the entry ladder cannot be reached lazily without a split chunk, which IIFE consumers cannot take — so the line is paid, not trimmed, and gated from here on at 29 KB / 9.5 KB.

## `@airo-js/embed` 0.11.0 — 2026-09-10

### Added
- **`EmbedPhase` gains `'gate'`.** A gate's `precheck` / `mount` throw reaches the host's `onError` as `'gate'` (a retry), forwarded distinctly like `'resolve-view'`, and reported once — until now it surfaced as `'mount'` (fatal) through the facade while the runtime README called it retryable.
- **`LoadConfigResult.satisfiedGates`** — the server-verified hand-off through the facade: a host that server-rendered a private page reads `data-airo-gates-satisfied` off its markup and returns the ids from `loadConfig`; `defineAiroApp` forwards them to `mountCartridge({ satisfiedGates })`, so the hydrate never re-asks the login gate. Initial mount only.

### Changed
- **Bundle budget raised: minified 5 KB → 5.25 KB** (gzip stays 2.5 KB). Measured after the two additions: 5,159 B minified / 2,248 B gzip, from 5,081 B / 2,211 B — +78 B for forwarding a phase and one option. The embed sat 39 bytes under its ceiling for several lines; this is the first growth since, and it is a deliberate changelog line, not a silent edit.
- A host that supplies `templatePages` replaces the template's pages wholesale, so a persisted page graph that omits `private` silently makes that page public — carry the flag through (stated in the upgrading note).
...
## Examples and docs — 2026-09-10

### Added
- **`full-site` has a members area.** Two `private: true` pages (`/members`, `/note/:slug`) behind a sign-in gate, with the whole of auth in the Express host: an in-process OAuth 2.0 identity provider (authorization code + PKCE S256, one registered client with an exact-match `redirect_uri`, one-shot 60 s codes, `demo` / `demo`) and a relying party (`/auth/login`, `/auth/callback`, `POST /auth/logout`, `GET /auth/session`; HttpOnly SameSite=Lax cookie, `Secure` behind a TLS terminator, same-origin `next` only, 502 with no cookie when the provider is unreachable). The server calls the runner **without** `renderPrivate` first and only on `skipped.reason === 'private'` reads the cookie: anonymous → a 401 shell (`data-airo-mode="csr"`, no bundle-free markup, `noindex` twice, `no-store`); a session → the page server-rendered adapter-free with `data-airo-gates-satisfied="login"`, `Cache-Control: private, no-store`, `Vary: Cookie`, `X-Robots-Tag: noindex`. The member slice is built only inside that branch, so a signed-in visitor's public pages are byte-identical to an anonymous visitor's and stay `public, max-age=60`. No inline snapshot block anywhere: every private mount refetches through the `members` DataSource with the cookie. `/api/members/me` is the authority; the gate is UX. Machine surfaces (`/sitemap.xml`, `/llms.txt`, `/mcp/*`) never read a cookie and cannot carry a member byte.
- **Tests for the example.** The smoke grows from 27 to 99 assertions (the round trip, every hardening check, the byte-identical public page, the provider-down 502 via a second server instance on a free port, the 401 shell's server-rendered panel, a client bundle free of member content); 33 unit tests under happy-dom and node with `fetch` mocked or a stub issuer (`test/gate.test.ts`: gate before data, satisfied hand-off, public entry never gated, session expiry between render and hydrate fetch, the session-ended panel, two concurrent mounts keeping their own precheck error, the no-paint attribute; `test/auth.test.ts` and `test/auth-issuer.test.ts`: the clock — session, code and token expiry — the callback's own refusals, a forged pending-login signature, malformed cookies, the provider's request validation, an issuer that answers with no identity, with HTML, or never; `test/views.test.ts`: the private views' signed-out state, the note view, the members DataSource, the transformer); `e2e/members.spec.ts` drives Chromium through sign-in, in-place hydration with exactly one `/api/members/me` and no `/auth/session`, the anonymous page with JavaScript disabled, deep link, cancel, wrong password, external `next`. The example's tests and specs are now typechecked (`tsconfig.test.json`), the trap cartridge-kit's vitest config documents.
- **Hardening from the pre-landing review**, all in the example's host code: the pending-login cookie is HMAC-signed (login CSRF via a planted cookie); `next` is capped at 1 KB (a longer one overflowed the cookie and every sign-in from that link failed with an unexplained state mismatch); a malformed `%`-escape in a cookie no longer throws a 500 out of every private route; both server-to-provider fetches time out at 5 s (a hung provider was a pinned worker, not a 502); a token response that is not JSON is a 502, not a stack trace; the PKCE and state checks run on the POST that mints the code, not only on the GET that renders the form; secrets, the demo password and the verifier digest compare in constant time; the session store is capped; bearer 401s carry `WWW-Authenticate`; the client id and secret read from `AUTH_CLIENT_ID` / `AUTH_CLIENT_SECRET`; the login gate's precheck error is per mount and its fetch times out; the note panel's `next` is percent-encoded once; the client entry catches the runtime's rethrow so an expired session leaves a panel and a marker rather than an unhandled rejection; the 401 shell server-renders the sign-in panel, so the anonymous page works without JavaScript; the member notes moved to a server-only module the client bundle cannot reach; an unknown url on a members-first template is a 404, never a 401; the private snapshot is transformed once; sitemap and `llms.txt` build in parallel and carry `Cache-Control`.

### Fixed
- **`full-site` never served its client bundle** — `server.ts` referenced `/client.js` and the route fell through to the wildcard, so the example's hydrate path had never run in a browser. esbuild now bundles `src/client.ts` to `dist/public/client.js`, served before the wildcard; `e2e/public.spec.ts` proves the index and a doc page hydrate in place.

...
- **Tests for the example.** The smoke grows from 27 to 97 assertions (the round trip, every hardening check, the byte-identical public page, the provider-down 502 via a second server instance); `test/gate.test.ts` mounts the cartridge under happy-dom with `fetch` mocked (gate before data, satisfied hand-off, public entry never gated, session expiry between render and hydrate fetch, the no-paint attribute); `e2e/members.spec.ts` drives Chromium through sign-in, in-place hydration with exactly one `/api/members/me` and no `/auth/session`, deep link, cancel, wrong password, external `next`.

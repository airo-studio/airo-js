...
Pre-1.0 hardening, staged for **0.9.1** — the `@airo-js/runtime` peer-dependency fix below is a defect in the published 0.9.0 and should not wait for 1.0.0 to reach consumers.

`@airo-js/mcp` joins the line rather than starting a version line of its own: it publishes for the first time at `0.9.1`, so 1.0.0 lands on every package at once instead of leaving one package visibly behind the release it belongs to. It ships pre-1.0 deliberately, so contributors can shake the surface out before the tag freezes it. `@airo-js/log` keeps its independent line (`0.3.x`).

### Added
- **`@airo-js/mcp` is a real package — the agent leg of the three-audience thesis was a 39-line interface.** `McpToolDefinition` shipped in cartridge-kit from the start and *nothing in the framework ever read it*: no manifest emitter, no dispatcher, no tests. The package that was supposed to own that was 11 lines of doc comment and a `PACKAGE_NAME` const, `private: true`, never published. Meanwhile all three cartridges in this repo's own examples hand-rolled the missing runner — `examples/shopify-edge-worker` carried two near-identical private copies (the `buildToolManifest as buildWpManifest` / `as buildShopifyManifest` aliasing in its worker imports was the tell) and `examples/full-site` inlined a third with a cast per field, because no typed helper existed. That is §7's placeholder-package trap, shipped as the reference material people copy.
  - **`buildToolManifest(cartridge, opts?)`** — MCP's `tools/list` payload and nothing more. `opts.toolNames` restricts the set; `opts.snapshot` additionally omits tools the snapshot cannot cover, **off by default** because MCP clients cache tool lists and a tool that appears and disappears as data changes is worse for an agent than one always listed that sometimes answers `missing-required-fields`, which at least names what is absent. The hand-rolled version in `full-site` silently dropped `inputSchema`; the framework's does not.
  - **`dispatchTool(cartridge, name, input, snapshot, opts)`** — resolve, gate on coverage, optionally validate input, invoke. Returns a discriminated union and **never throws**: every outcome becomes a protocol response, and an agent calling a stale tool name is ordinary rather than exceptional. Both hand-rolls threw, so an unknown tool name reached the edge worker as an unhandled rejection and a 500; the same request is now the 400 it always was. `ctx.schema` comes from `cartridge.schema`, retiring the passthrough stub both examples had invented to satisfy `ToolContext`.
  - Input validation is a **seam** (`validateInput`), not a bundled JSON Schema validator — ajv is ~30 KB and the choice belongs to a host that already has one. Omitting it skips validation, documented rather than silent.
  - Scope is emission and dispatch only. No transport, auth, rate limiting, tenancy or session state; `ToolContext.scope` carries a host's tenancy through opaquely and the framework never reads it.
- **`McpToolDefinition.requires`** — optional coverage metadata, gated by the same shared predicate as `PublicationAdapter.requires`, so a feed and an agent answer reading the same field cannot reach opposite verdicts about whether it is answerable. Optional rather than required because most tools read the snapshot broadly, and because adding a required field to a published interface would be breaking.
- **`missingRequiredPaths(requires, snapshot)`** on `@airo-js/cartridge-kit` — the coverage predicate itself, extracted so `runPublicationAdapters` and `dispatchTool` cannot drift on what `'always'` and "present" mean. That drift is exactly what the snapshot-fidelity guarantee exists to prevent, so it is one function rather than one per consumer.
- **`@airo-js/core` has tests.** The largest package (3,322 LOC) and the dependency root shipped through 0.9.0 with no test file of its own. Its integration paths were genuinely exercised — 30 test files in the other packages reach through it — but five public exports had zero mentions anywhere in the workspace, which is a poor position from which to freeze a 1.0 surface. 107 tests now cover `runtime-pipeline` (the DataSource→Transformer→Gate→PostProcessor engine: `errorPolicy: 'skip'` proven to pass the *input* through rather than a partial result, since §6's "never 'skip' a visibility filter" rule depends on exactly that; LIFO teardown; one throwing hook not taking down its siblings in either direction; and the traced and untraced paths separately, because `!trace && policy === 'fail-render'` skips the try/catch entirely and the two are different code), `parse-html`, `theme`, `breadcrumb` and `style`. `parse-html`'s suite runs in `node` **on purpose** — under a DOM environment a global `document` exists and every call would silently take the fallback branch, leaving the explicit-document path the docs actually recommend unproven, and the "no Document available" throw unreachable. Verified non-vacuous by mutation: reversing the teardown order, making `'skip'` return a partial, and injecting a framework default stylesheet each fail three tests.
- **Lint gate.** `pnpm lint` runs Biome (`biome.jsonc`) with `--error-on-warnings`, so a new warning fails rather than accumulating; `pnpm lint:fix` applies safe fixes. Biome over ESLint for one binary and no plugin graph — this repo carried a single devDependency before the pre-1.0 pass, and a small toolchain is worth something to a framework people vendor into their own builds. **Linter only, formatter off:** enabling the formatter would rewrite every file, bury real changes in whitespace churn and flatten hand-laid alignment in table-shaped comments; adopting it deserves its own commit rather than arriving as a side effect. Four rules are disabled, each with its reason in the config: `noNonNullAssertion` contradicts `noUncheckedIndexedAccess: true`, which is the stronger guarantee and the one worth keeping; `noConfusingVoidType` fires on `HydrateCleanup | void`, the deliberate public shape of the cleanup-callback contract; `useTemplate` and `useLiteralKeys` are pure style, and Biome marks every `useLiteralKeys` fix here as unsafe.

### Fixed (found by the new lint gate)
- **`Object.prototype.hasOwnProperty.call(b, k)` → `Object.hasOwn(b, k)`** in `mount-cartridge`'s deep-equal. The `.call` form was already the safe one; `Object.hasOwn` is the modern equivalent and the package targets Node ≥ 20.
- **`isNaN` → `Number.isNaN`** in the Shopify example's price validation. Not a live defect — the argument was already `Number(...)`-wrapped — but `isNaN` coerces and should not be reached for by default in code people copy.
- **A private `escape()` in `@airo-js/core` shadowed the deprecated global `escape()`**, in the one file where a reader is most likely to wonder which one they are looking at. Renamed `escapeEntities`; the exported `escapeHtml` / `escapeAttr` are unchanged.
- Two `!x || !x.y` guards in `PageManager` collapsed to `!x?.y`, a stale `lifecycleB` left over from an earlier two-cartridge draft of a runtime test, an unused `afterEach`-adjacent import, and two unused parameters marked `_`.

- **Dead-code gate.** `pnpm knip` runs in CI mode (dead files, dead internal exports, unlisted/unused dependencies); `pnpm knip:audit` adds `--include-entry-exports` to list every public export with no consumer anywhere in the repo — the "what are we freezing at 1.0" review rather than a gate. Config in `knip.json`. Added before 1.0 specifically because an accidental export becomes a semver commitment the moment the tag lands, and cannot be retracted until 2.0.
- **`AdapterSkipped`** exported from `@airo-js/ssr`, and `AdapterRunResult.skipped`. Present only on an adapter that coverage gating stopped, so a caller can tell "ran and failed validation" from "never ran" — `included: false` alone cannot distinguish them.
- **`UpdateResult` re-exported from `@airo-js/runtime`'s barrel.** It is the return type of `MountCartridgeResult.update` / `.updatePages`, so typing the result of a call you made against `@airo-js/runtime` no longer means importing from a package you never called.

### Fixed
- **`@airo-js/runtime` declared `@airo-js/log` as a devDependency while importing it at module scope.** `dist/mount-cartridge.js` carries an unconditional `import { initLogControls, logger } from '@airo-js/log'`, but npm does not install a devDependency for consumers, and runtime was the only package on the line not declaring log as a peer (core, ssr and embed all did). Anyone installing `@airo-js/runtime` without another package resolving log for them hits a module-resolution failure at import time. Now a peerDependency. Present in the published 0.9.0.
- **`PublicationAdapter.requires` is now enforced — contract guarantee #2 was documented from the start and never implemented.** `runPublicationAdapters` checks each adapter's `required: 'always'` paths against the snapshot before calling `generate()`; a starved adapter does not run, and its result carries `included: false` plus `skipped: { reason: 'missing-required-fields', missing }`. Two deliberate narrowings. Only `'always'` gates — whether a missing `'preferred'` field should stop publication is a judgment the adapter makes in `validate()`, where it can see the output it produced. And **present means non-nullish**, not `hasByPath`'s declared-key semantic: `''`, `0`, `false` and `[]` all count as present, because an adapter that considers an empty string unusable says so in `validate()`, while a feed cannot emit `undefined` either way. The gate ignores `onValidationFail` — that policy governs output that failed `validate()`, and a skipped adapter produced none, so a skip never throws even under `'fail-loud'`. **Behaviour change:** an adapter declaring `'always'` paths its snapshot does not populate now yields a skip where it previously yielded output.
- **`UpdateResult`'s back-compat re-export had been inert since 0.7.1.** `mount-cartridge.ts` carried `export type { UpdateResult }` with a comment stating that 0.7.0 consumers importing it from `@airo-js/runtime` keep working. They did not: the barrel never forwarded it and `exports` exposes only `.` and `./test-harness`, so the module path it lived on was unreachable. Caught by knip.

### Changed
- **`@airo-js/log`'s tests moved into `@airo-js/log`.** `log-controls` and `log-console-format` (35 tests) lived in `packages/runtime/test/`, so log — which is on its own version line — could not gate its own release, and a runtime-only run was the only thing exercising it. The jsdom parity config moved with them rather than being left behind: `initLogControls` reads `window.location` and writes `localStorage`, and it is called directly by consumers whose suites run under jsdom, so a control surface that works in happy-dom and breaks in jsdom is a shipped break that only a second environment catches. The sanitizer-compat guard (which statically asserts the `localStorage.setItem` key is the literal `'__airo_log'` rather than a const — the 0.3.0 publish blocker) needed its source path re-pointed from `../log/src/index.ts` to `src/index.ts`, and was re-verified by mutation after the move.

- **best-practices §2.5c: the typecheck claim was false for the pattern the section exists to serve.** It stated that typing the shared allowlist as an interface "moves the failure from runtime to build time, which is the only version of hand-maintenance that is safe." That holds only when a chunk reads the global explicitly and therefore names the type. Under the `output.globals` externals mapping — the natural pairing with this pattern — a chunk writes an ordinary `import { getByPath } from '@airo-js/cartridge-kit'`, which typechecks clean against the real package because the export genuinely exists; the build rewrites it afterwards, and the interface constrains only the core's own assignment. One side of a two-sided contract. A consumer adopting the documented pattern shipped an allowlist missing three symbols, passed `tsc --noEmit`, passed 2,179 tests, built inside budget, and died at render on every chunked page. The section now scopes the claim and carries the guard that does work: Rollup's `importedBindings`, which records the exact bindings each chunk took from each external, so the check needs no source scanning. Contributed by the consumer who hit it.
- **§2.5c's measurements were from the broken build.** The figures landed in `ab85ed1` came from that same five-symbol allowlist — the one missing `getByPath`, `setByPath` and `defineStyleSurface`. Corrected to the consumer's eight-symbol numbers: 91.3 KB raw / 27.7 KB gzip / 2.8 KB headroom, 2.5 KB recovered, 0.8 KB below their 0.8.8 baseline, eight functions rather than five. The argument is unchanged and slightly smaller in magnitude.
- **`defineCrawlerSurfaceAdapter`'s note that `requires` is unenforced is retired,** along with its claim that enforcement was blocked on schema-space→snapshot-space path resolution. `SchemaDefinition<TData>.parse` returns `TData`, so a schema path and a snapshot path name the same place and `getByPath` resolves both.

### Removed
- `__resetRegisteredElementsForTesting` from `@airo-js/embed`. A test-only escape hatch whose docstring said tests import it directly; no test ever did — they use `uniqueElementName()` instead, which avoids the shared mutable state the hatch existed to reset. Never on the package barrel, so not a public-surface change.

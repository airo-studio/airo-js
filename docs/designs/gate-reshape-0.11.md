...
## Consumer evaluation (after the review, before the cut)

Both consumers read this doc against their own code and accepted the direction. Four corrections came back and are folded into the line:

- **The step 4 tracer bullet could not run as written.** The studio consumer is not yalc-linked (it consumes published versions and copies store artifacts by hand for pre-release checks), so a `yalc push` would have reported success and reached nobody. Replaced: the consumers run their own suites against the `gate-reshape-0.11` branch and report on their threads; a prerelease tag is the alternative if the owner authorises a publish.
- **D15's justification was wrong even though the mechanism is right.** The studio consumer's age modal is appended beside the render root, not into it, so the hydrate restore is a no-op for them. The runtime keeps the guarantee for the case it covers (a gate that paints into the render root and then allows) and the docs describe it as the synthetic contract it is. The wider rule is "undo what you did, anywhere in the document", not only "what you did to the host".
- **`persist` is the third declared-and-unread primitive on this line.** Its docblock now opens by saying the framework never reads it; whether it survives 1.0 as a field is on the freeze review list.
- **The site consumer's server shape is cleaner than the example's.** Call the runner without `renderPrivate` first; on `skipped.reason === 'private'` verify the cookie, build the private slice, call again with `renderPrivate: true`. The host never re-derives the entry, and "public pages never read the cookie" holds by construction. The example (step 6b) adopts it and §4.9 shows it. Three more sentences from that reply land in §4.9: authenticated-but-not-entitled is a private render with an empty slice (never a 403 that tells an anonymous caller which guess was warm); a template whose first enabled page is private has a private default entry, so `/` refuses anonymously (pinned by test — keep a public page first unless you mean it); a host that co-locates tool dispatch with the render splits the two on a private request so tools never see the private slice.

Also from the replies: D24 (a studio dev-mode gate memo) is closable — their precheck is a synchronous storage read, so per-remount cost is nil; `gates.pending` is coarse under a one-gate-fan-out (it names the one framework gate) and the docs say so; and the field names the studio consumer waits on for their SSR output are the ones in code: `data-airo-gate` = `pending` (host) → `passed` | `blocked` | `error` (runtime), `gates: { pending: string[]; satisfied: string[] }` on the SSR result, `data-airo-gates-satisfied` on the mount root, `mountCartridge({ satisfiedGates })`.

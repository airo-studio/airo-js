...
**`onShellReady` runs before the render phase — guaranteed.** The mount sequence is shell → `onShellReady` → entry → gates → data → pipeline → mount, and `onShellReady` is synchronous. A `shell.events` subscription made inside it observes every emission from the render/hydrate phase, including events components fire during their initial render (auto-fire on mount, hydrate-phase misses). This is documented contract, not observed behavior — build event bridges (analytics, debug observers) on it freely. Equivalent alternative: pre-build the bus and pass it via `options.events` as shown above.
...
**Diff before you dispatch — and compare arrays structurally.** Hosts that translate a full editor/config state and then diff it to build the delta must use structural comparison, not reference equality — arrays especially. A rebuilt-but-equal array (fresh reference every editor render) that lands in the delta becomes an uncovered leaf path, and the classifier correctly routes it to remount — so `hotSwapKeys` never gets a chance and every panel edit pays a full pipeline replay. If your hot-swap paths "never hot-swap," audit the host-side diff first: the framework's own classifier already treats structurally-equal values as unchanged (`diffLeafPaths` deep-equal short-circuit); a naive host diff upstream of `update()` defeats it.

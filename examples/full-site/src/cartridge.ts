 ...
 * public pages and a members area alike. This is its BROWSER half.
 *
 ...
 * data sources, transformer, renderers — and, in `cartridge.server.ts`, the
 * MCP tools and the publication adapters covering every other audience.
 * What changed is that it now spans SEVERAL pages, is tracked, runs, uses
 * the 0.9.0 primitives instead of hand-rolling the crawler bundle, carries
 * two PRIVATE pages behind a sign-in gate (0.11.0), and keeps its adapters
 * and tools out of the browser bundle (0.11.1).
 *
 * `client.ts` imports this file; `server.ts` imports the server half, which
 * spreads this cartridge and adds the rest. Nothing in this file may import
 * the server half.
 *
...
const contentSource: DataSource<DocSiteData, DocSiteConfig> = {
  ...
  async fetch(input): Promise<DocSiteData> {
    ...
    //
    // Parsed even though the content is in-process: the framework never
    // calls `schema.parse()`, so a DataSource that skips it hands views
    // whatever it built. Swap `DOCS` for a CMS and this line is the check.
    return docSiteSchema.parse({ site: SITE, index, ...(slug ? { doc: findDoc(slug) } : {}) });
  },
};

...
| Package | Role |
|---|---|
| `@airo-js/core` | Runtime engine — `createApp`, `PageManager`, `EventBus`, `Theme`, `IsolationRoot`, `registry`. |
| `@airo-js/runtime` | Layout chunk loader + hydration glue between core and per-page bundles. |
| `@airo-js/ssr` | Edge SSR pipeline — `renderAppToHTML` + runtime-agnostic dispatch. |
| `@airo-js/embed` | ~5 KB bootstrap loader — what customers paste in their HTML. |
| `@airo-js/mcp` | MCP tool emission helpers — cartridge-defined tools surface here. |
| `@airo-js/cartridge-kit` | The cartridge contract: `Cartridge<TSchema, TConfig>` + DataSource / View / MCP-tool / Template primitives. |

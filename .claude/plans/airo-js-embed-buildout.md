...
export interface DefineAiroAppOptions {
   ...
   * 'wtb-widget'. Host apps pick a name that fits their brand.
   */
  elementName?: string;
  /**
   ...
   *     if (id === 'wtb') return (await import('@my-org/wtb-cartridge')).wtbCartridge;
...
defineAiroApp({
  ...
  loadConfig: async (id, token) => {
    ...
    return {
      config: body.config,
      cartridgeId: body.config.cartridge?.id ?? 'wtb',
  ...
  resolveCartridge: async (id) => {
    if (id === 'wtb') {
      const mod = await import('@/cartridges/wtb');
      return mod.wtbCartridge;
    }
    throw new Error(`unknown cartridge: ${id}`);
  },

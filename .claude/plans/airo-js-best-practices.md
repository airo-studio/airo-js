...
- The cartridge port in `dotter-monorepo/apps/dotter-studio/src/cartridges/wtb/` (the second consumer that pressure-tested everything)
...
const ageGate: Gate<WtbConfig> = {
  ...
  precheck: async () => {
    const verified = readSessionStorage('wtb:age-verified') === 'true';
  ...
  destroy: () => undefined,
  persist: { key: 'wtb:age-verified', scope: 'session' },
};
```
...
```
v1 (wrong):                     v0 (right):
wtb-storeplus.js (96 KB)        wtb-categories.js  (~25 KB)
└─ all 4 sub-pages              wtb-products.js    (~50 KB)
   bundled together             wtb-product.js     (~40 KB)
                                wtb-quickview.js   (~25 KB)
```
...
[`dotter-monorepo/apps/dotter-studio/src/cartridges/wtb/`](file:///Users/smithbn/workspace/dotter-monorepo/apps/dotter-studio/src/cartridges/wtb/) — full WTB cartridge port. Survived three formal reviews. Per-page chunks, two-envelope schema, Gate primitive, page-config adapters, all six v0 components lifted.
...
- `parts/wtb-data.ts` + `parts/schema.ts` — type definitions + browser-safe SchemaDefinition stub.
...
- `dotter-monorepo/.claude/plans/airo-cartridge-kit-spike-findings.md` — six findings from the WTB consumer mapping.
